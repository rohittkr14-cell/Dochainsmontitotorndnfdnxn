...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot is Live and running successfully!")

    def log_message(self, format, *args):
        # Keeps logs clean by ignoring repetitive GET requests from Render
        return

def run_health_check_server():
    port = int(os.environ.get("PORT", 10000))  # Render's dynamic port or default 10000
...
async def main():
    ...
    # Render's Health Check Server started in a separate thread
